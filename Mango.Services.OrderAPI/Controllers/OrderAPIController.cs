    ...
    ) : ControllerBase
{
    ...
    public ResponseDto? Get(string? userId = "")
    {
        try
        {
            ...
            {
                objList = _dbContext.OrderHeader.Include(u => u.OrderDetails).OrderByDescending(u => u.OrderHeaderId).ToList();
            }
            else
            {
                objList = _dbContext.OrderHeader.Include(u => u.OrderDetails).Where(u => u.UserId == userId).OrderByDescending(u => u.OrderHeaderId).ToList();
            }
    ...
    public ResponseDto? Get(int id)
    {
        try
        {
            OrderHeader orderHeader = _dbContext.OrderHeader.Include(u => u.OrderDetails).First(u => u.OrderHeaderId == id);
    ...
    public async Task<ResponseDto> UpdateOrderStatus(int orderId, [FromBody] string newStatus)
    {
        try
        {
            OrderHeader orderHeader = _dbContext.OrderHeader.First(u => u.OrderHeaderId == orderId);
            if (orderHeader != null)
            {
